...
public class TaskChecker : MonoBehaviour
{
    ...
    TaskChecker()
    {
        ...
        taskList.Add(new Task("Navigate into the miners folder", "cd miners", "/root"));
        taskList.Add(new Task("Create a file called file.txt in the miners folder", "touch file.txt", "/root/miners"));
        taskList.Add(new Task("Print 'rush'", "echo rush", "/root"));
    ...
    {
        if (command == taskList[0].commandToRun && directory == taskList[0].workingDirectory)
        {
            OnTaskCompleted();
        }
    }
